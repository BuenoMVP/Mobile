...
import { prepararDadosGrafico, buscarDadosFirebase } from '../utils/dadosRelatorio';
...
const Relatorio = ({ route }) => {
    ...
    useEffect(() => {
        const carregarDados = async () => {
            try {
                ...
                const dados = await buscarDadosFirebase(id);
    ...
    const totalRespostas = Object.values(dadosPesquisa).reduce((acc, val) => acc + val, 0);
        ...
        return (
            <View style={estilos.errorContainer}>
                ...
                <Text style={estilos.errorMessage}>
                    Não foi possível carregar os dados da pesquisa. Verifique sua conexão com a internet e tente novamente.
                </Text>
            </View>
        );
    }
